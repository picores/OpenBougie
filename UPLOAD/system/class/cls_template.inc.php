...
class TPL
{
	...
	public static function init()
	{
		...
		{			
			self::$template_path = realpath(ROOT_PATH . 'themes/');
			...
			if (file_exists(BOG_PATH . 'config.inc.php') AND class_exists('BOP_APP', false))
			{
				self::$in_app = true;
			}
		}
	...
	public static function output($template_filename, $display = true)
	{
		...
		if (self::$in_app)
		{			
			if ($plugins = BOP_APP::plugins()->parse($_GET['app'], $_GET['c'], $_GET['act'], str_replace(self::$template_ext, '', $template_filename)))
			{
