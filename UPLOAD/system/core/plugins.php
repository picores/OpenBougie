	    ...
	    while (($file = readdir($dir_handle)) !== false)
	    {
	       	if ($file != '.' AND $file != '..' AND is_dir($this->plugins_path . $file))
	       	{
	            ...
	            if (file_exists($config_file))
	            {
		            ...
		            if (is_array($bop_plugin) AND G_VERSION_BUILD >= $bop_plugin['requirements'])
		            {		            	
		            	if ($bop_plugin['contents']['model'])
		            	{
			            	$this->plugins_model[$bop_plugin['contents']['model']['class_name']] = $this->plugins_path . $file . '/' . $bop_plugin['contents']['model']['include'];
		            	}
		            	
		            	if ($bop_plugin['contents']['setups'])
		            	{
			            	foreach ($bop_plugin['contents']['setups'] AS $key => $data)
			            	{
		            	...
		            	if ($bop_plugin['contents']['actions'])
		            	{
			            	foreach ($bop_plugin['contents']['actions'] AS $key => $data)
			            	{
			            		if ($data['app'] AND $data['controller'] AND $data['include'])
			            		{
