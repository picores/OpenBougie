...
class core_cache
{
	...
	public function __construct()
	{
		...
		BOP_APP::debug_log('cache', null, 'Backend: ' . $this->backendName);
	...
	{		
		if (BOP_APP::config()->get('system')->debug)
		{
		...
		if (BOP_APP::config()->get('system')->debug)
		{
			...
			BOP_APP::debug_log('cache', $stime, 'Save Cache: ' . $key . ', result type: ' . gettype($value));
		}
	...
	public function get($key)
	{
		...
		if (BOP_APP::config()->get('system')->debug)
		{
			...
			BOP_APP::debug_log('cache', $stime, 'Get Cache: ' . str_replace($this->groupPrefix, '', $key) . ', result type: ' . gettype($result));
		}
