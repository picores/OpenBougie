...
class AWS_CONTROLLER
{
	...
	{		
		...
		BOP_APP::session()->permission = $this->user_info['permission'];
		
		if ($this->user_info['forbidden'] == 1)
		{
			$this->model('account')->logout();
			
			H::redirect_msg(BOP_APP::lang()->_t('抱歉, 你的账号已经被禁止登录'), '/');
		}
		else
		{
		...
		{
			unset(BOP_APP::session()->human_valid);
		}
		else if ($this->user_info['permission']['human_valid'] and ! is_array(BOP_APP::session()->human_valid))
		{
			BOP_APP::session()->human_valid = array();
		}
		...
		if ($plugins = BOP_APP::plugins()->parse($_GET['app'], $_GET['c'], 'setup'))
		{
...
class AWS_ADMIN_CONTROLLER extends AWS_CONTROLLER
{
	...
	public function __construct()
	{
		...
		$admin_info = H::decode_hash(BOP_APP::session()->admin_login);
		
		if ($admin_info['uid'] != $this->user_id OR $admin_info['UA'] != $_SERVER['HTTP_USER_AGENT'] OR $admin_info['ip'] != fetch_ip() OR !BOP_APP::session()->permission['is_administortar'])
		{
			unset(BOP_APP::session()->admin_login);
			
			if ($_POST['_post_type'] == 'ajax')
			{
				H::ajax_json_output(BOP_APP::RSM(null, -1, BOP_APP::lang()->_t('会话超时, 请重新登录')));
			}
			else
			{
