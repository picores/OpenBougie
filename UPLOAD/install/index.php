...
if (file_exists(BOG_PATH . 'config/install.lock.php'))
{
...
switch ($_POST['step'])
{
	default :
		...
		// 检测 BOG_PATH 是否有写权限
		if (is_really_writable(BOG_PATH) OR defined('IN_SAE'))
		{
			$system_require['config_writable_core'] = TRUE;
		}
		
		// 检测 BOG_PATH /config/ 是否有写权限
		if (is_really_writable(BOG_PATH . 'config/') OR defined('IN_SAE'))
		{
	...
	case 2 :
		if (!defined('IN_SAE'))
		{
			...
			if (! is_really_writable(BOG_PATH))
			{
				$error_messages[] = load_class('core_lang')->_t('目录: %s 无法写入，请将此目录权限设置为 777', BOG_PATH);
			}
		}
	...
	case 4 :		
		...
		if (!defined('IN_SAE'))
		{
			$config_file = file_get_contents(BOG_PATH . 'config.dist.php');
			...
			file_put_contents(BOG_PATH . 'config.inc.php', $config_file);
			file_put_contents(BOG_PATH . 'config/install.lock.php', time());
		}
