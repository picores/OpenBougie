...
class qq extends AWS_CONTROLLER
{
	...
	function binding_weibo_action()
	{
		...
		{
			H::redirect_msg(BOP_APP::lang()->_t('腾讯微博绑定功能已关闭'), '/');
		}
	...
	function callback_weibo_action()
	{
		...
		{
			H::redirect_msg(BOP_APP::lang()->_t('与微博通信出错, 请重新登录'), '/account/setting/openid/');
		}
	}
	...
	function binding_qq_action()
	{
		...
		{
			H::redirect_msg(BOP_APP::lang()->_t('QQ 帐号绑定功能已关闭'), '/');
		}
		
		unset(BOP_APP::session()->QQConnect);
	...
	function callback_qq_action()
	{
		...
		if (! $_GET['code'])
		{
			H::redirect_msg(BOP_APP::lang()->_t('与 QQ 通信出错, 请重新登录'), '/account/login/');
		}
		
		if (! BOP_APP::session()->QQConnect['access_token'])
		{
		...
		if (! BOP_APP::session()->QQConnect['access_token'] OR ! $uinfo = $this->model('openid_qq')->request_user_info())
		{
			H::redirect_msg(BOP_APP::lang()->_t('与 QQ 通信出错, 请重新登录'), '/account/login/');
		}
		else
		{
			if (!$this->model('integral')->fetch_log($this->user_id, 'BIND_OPENID'))
			{
				$this->model('integral')->process($this->user_id, 'BIND_OPENID', round((get_setting('integral_system_config_profile') * 0.2)), BOP_APP::lang()->_t('绑定 OPEN ID'));
			}
