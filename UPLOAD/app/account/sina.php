...
class sina extends AWS_CONTROLLER
{
	...
	{	
		...
		BOP_APP::session()->sina_token = $oauth->getAccessToken('code', array(
			'code' => $_GET['code'],
			'redirect_uri' => get_js_url('/account/sina/binding_callback/')
		));
		
		$client = new Services_Weibo_WeiboClient(get_setting('sina_akey'), get_setting('sina_skey'), BOP_APP::session()->sina_token['access_token']);
		...
		{
			H::redirect_msg(BOP_APP::lang()->_t('与微博通信出错, 错误代码: %s', $sina_profile['error']), "/account/setting/openid/");
		}
