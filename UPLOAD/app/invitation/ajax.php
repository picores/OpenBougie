...
class ajax extends AWS_CONTROLLER
{
	...
	public function invite_action()
	{
		...
		{
			H::ajax_json_output(BOP_APP::RSM(null, -1, BOP_APP::lang()->_t('当前帐号没有提供 Email, 此功能不可用')));
		}
		
		if (! H::valid_email($_POST['email']))
		{
			H::ajax_json_output(BOP_APP::RSM(null, -1, BOP_APP::lang()->_t('请填写正确的邮箱')));
		}
		
		if (! $this->user_info['invitation_available'])
		{
			H::ajax_json_output(BOP_APP::RSM(null, -1, BOP_APP::lang()->_t('已经没有可使用的邀请名额')));
		}
		...
		if ($uid = $this->model('account')->check_email($_POST['email']))
		{
			if ($uid == $this->user_id)
			{
				H::ajax_json_output(BOP_APP::RSM(null, -1, BOP_APP::lang()->_t('你不能邀请自己')));
			}
			
			H::ajax_json_output(BOP_APP::RSM(null, -1, BOP_APP::lang()->_t('此邮箱已在本站注册帐号')));
		}
		...
		if ($invitation_info = $this->model('invitation')->get_invitation_by_email($_POST['email']))
		{
			if ($invitation_info['uid'] == $this->user_id)
			{
				...
				H::ajax_json_output(BOP_APP::RSM(null, 1, BOP_APP::lang()->_t('重发邀请成功')));
			}
			else
			{
				H::ajax_json_output(BOP_APP::RSM(null, -1, BOP_APP::lang()->_t('此邮箱已接收过本站发出的邀请')));
			}
		}
		...
		if ($invitation_id = $this->model('invitation')->add_invitation($this->user_id, $invitation_code, $_POST['email'], time(), ip2long($_SERVER['REMOTE_ADDR'])))
		{
			...
			H::ajax_json_output(BOP_APP::RSM(null, 1, BOP_APP::lang()->_t('邀请发送成功')));
		}
	}
	...
	public function invite_cancel_action()
	{
		...
		{
			H::ajax_json_output(BOP_APP::RSM(null, -1, BOP_APP::lang()->_t('邀请记录不存在')));
		}
