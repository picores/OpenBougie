...
class main extends AWS_ADMIN_CONTROLLER
{
	public function index_action()
	{
		...
		$this->crumb(BOP_APP::lang()->_t('管理中心首页'), "admin/main/");
	...
	public function login_action()
	{
		...
		{
			H::redirect_msg(BOP_APP::lang()->_t('你没有访问权限, 请重新登录'), '/');
		}
		else if (BOP_APP::session()->admin_login)
		{
	...
	public function login_process_ajax_action()
	{
		...
		if (! $this->user_info['permission']['is_administortar'])
		{
			H::ajax_json_output(BOP_APP::RSM(null, '-1', BOP_APP::lang()->_t('你没有访问权限, 请重新登录')));
		}
		
		if (get_setting('admin_login_seccode') == 'Y' AND !BOP_APP::captcha()->is_validate($_POST['seccode_verify']))
		{
			H::ajax_json_output(BOP_APP::RSM(null, -1, '请填写正确的验证码'));
		}
		...
		{
			H::ajax_json_output(BOP_APP::RSM(null, '-1', BOP_APP::lang()->_t('帐号或密码错误')));
		}
	}
