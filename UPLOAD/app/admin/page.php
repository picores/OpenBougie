...
class page extends AWS_ADMIN_CONTROLLER
{
	...
	{
		$this->crumb(BOP_APP::lang()->_t('页面管理'), "admin/page/");
	...
	{
		$this->crumb(BOP_APP::lang()->_t('添加页面'), "admin/page/add/");
	...
	{
		$this->crumb(BOP_APP::lang()->_t('编辑页面'), "admin/page/edit/");
		
		if (!$page_info = $this->model('page')->get_page_by_url_id($_GET['id']))
		{
			H::redirect_msg(BOP_APP::lang()->_t('页面不存在'));
		}
	...
	public function page_add_action()
	{
		...
		if (!$_POST['url_token'])
		{
			H::ajax_json_output(BOP_APP::RSM(null, '-1', BOP_APP::lang()->_t('请输入页面 URL')));
		}
		
		if (!preg_match("/^(?!__)[a-zA-Z0-9_]+$/i", $_POST['url_token']))
		{
			H::ajax_json_output(BOP_APP::RSM(null, '-1', BOP_APP::lang()->_t('页面 URL 只允许输入英文或数字')));
		}
		
		if ($this->model('page')->get_page_by_url_token($_POST['url_token']))
		{
			H::ajax_json_output(BOP_APP::RSM(null, '-1', BOP_APP::lang()->_t('已经存在相同的页面 URL')));
		}
	...
	public function page_edit_action()
	{
		...
		{
			H::ajax_json_output(BOP_APP::RSM(null, '-1', BOP_APP::lang()->_t('页面不存在')));
		}
	...
	public function save_enabled_status_action()
	{
		...
		H::ajax_json_output(BOP_APP::RSM(null, -1, BOP_APP::lang()->_t('启用状态已自动保存')));
	}
}