...
class edm extends AWS_ADMIN_CONTROLLER
{
	...
	{
		$this->crumb(BOP_APP::lang()->_t('邮件群发'), "admin/edm/");
	}
	...
	public function add_group_action()
	{
		...
		if (trim($_POST['title']) == '')
		{
			H::ajax_json_output(BOP_APP::RSM(null, '-1', BOP_APP::lang()->_t('请填写用户群名称')));
		}
		...
		H::ajax_json_output(BOP_APP::RSM(null, 1, BOP_APP::lang()->_t('用户群添加完成')));
	}
	...
	public function add_task_action()
	{
		if (trim($_POST['title']) == '')
		{
			H::ajax_json_output(BOP_APP::RSM(null, -1, BOP_APP::lang()->_t('请填写任务名称')));
		}
					
		if (trim($_POST['subject']) == '')
		{
			H::ajax_json_output(BOP_APP::RSM(null, -1, BOP_APP::lang()->_t('请填写邮件标题')));
		}
					
		if (intval($_POST['usergroup_id']) == 0)
		{
			H::ajax_json_output(BOP_APP::RSM(null, -1, BOP_APP::lang()->_t('请选择用户群组')));
		}
		...
		H::ajax_json_output(BOP_APP::RSM(null, 1, BOP_APP::lang()->_t('任务建立完成')));
	...
	public function remove_task_action()
	{
		...
		H::redirect_msg(BOP_APP::lang()->_t('任务已删除'), get_setting('base_url') . '/?/admin/edm/tasks/');
	}
	...
	public function remove_group_action()
	{
		...
		H::redirect_msg(BOP_APP::lang()->_t('用户群已删除'), get_setting('base_url') . '/?/admin/edm/groups/');
	}
	...
	public function export_active_users_action()
	{
		...
		{
			H::redirect_msg(BOP_APP::lang()->_t('没有活跃用户'));
		}
	}
}