...
class feature extends AWS_ADMIN_CONTROLLER
{
	...
	public function list_action()
	{
		...
		$this->crumb(BOP_APP::lang()->_t('专题管理'), "admin/feature/list/");
		
		TPL::assign('pagination', BOP_APP::pagination()->initialize(array(
	...
	{		
		$this->crumb(BOP_APP::lang()->_t('添加专题'), "admin/feature/add/");
	...
	{		
		$this->crumb(BOP_APP::lang()->_t('编辑专题'), "admin/feature/list/");
	...
	public function topic_list_action()
	{
		...
		$this->crumb(BOP_APP::lang()->_t('专题话题管理'), "admin/feature/list/");
	...
	public function save_ajax_action()
	{
		...
		if (trim($_POST['title']) == '')
		{
			H::ajax_json_output(BOP_APP::RSM(null, -1, BOP_APP::lang()->_t('专题标题不能为空')));
		}
		...
		if ($_POST['url_token'])
		{
			...
			{
				H::ajax_json_output(BOP_APP::RSM(null, '-1', BOP_APP::lang()->_t('专题别名只允许输入英文或数字')));
			}
			
			if (preg_match("/^[\d]+$/i", $_POST['url_token']) AND ($feature_id != $_POST['url_token']))
			{
				H::ajax_json_output(BOP_APP::RSM(null, '-1', BOP_APP::lang()->_t('专题别名不可以全为数字')));
			}
		
			if ($this->model('feature')->check_url_token($_POST['url_token'], $feature_id))
			{
				H::ajax_json_output(BOP_APP::RSM(null, '-1', BOP_APP::lang()->_t('专题别名已经被占用请更换一个')));
			}
		}
		...
		if ($_FILES['icon']['name'])
		{
			BOP_APP::upload()->initialize(array(
			...
			if (BOP_APP::upload()->get_error())
			{
				switch (BOP_APP::upload()->get_error())
				{
					default:
						H::ajax_json_output(BOP_APP::RSM(null, '-1', BOP_APP::lang()->_t('错误代码') . ': ' . BOP_APP::upload()->get_error()));
					break;
					
					case 'upload_invalid_filetype':
						H::ajax_json_output(BOP_APP::RSM(null, '-1', BOP_APP::lang()->_t('文件类型无效')));
					break;	
				}
			}
			
			if (! $upload_data = BOP_APP::upload()->data())
			{
				H::ajax_json_output(BOP_APP::RSM(null, '-1', BOP_APP::lang()->_t('上传失败, 请与管理员联系')));
			}

			foreach (BOP_APP::config()->get('image')->feature_thumbnail as $key => $val)
			{
				$thumb_file[$key] = $upload_data['file_path'] . $feature_id . "_" . $val['w'] . "_" . $val['h'] . '.jpg';
				
				BOP_APP::image()->initialize(array(
					'quality' => 90,
		...
		H::ajax_json_output(BOP_APP::RSM(array(
	...
	public function delete_feature_ajax_action()
	{
		...
		H::ajax_json_output(BOP_APP::RSM(null, 1, null));
	}
}