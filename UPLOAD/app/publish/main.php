...
class main extends AWS_CONTROLLER
{
	...
	{
		$this->crumb(BOP_APP::lang()->_t('发布'), '/publish/');
	}
	...
	public function index_action()
	{
		if ($_GET['id'])
		{
			...
			{
				H::redirect_msg(BOP_APP::lang()->_t('指定问题不存在'));
			}
			...
			if (!$this->user_info['permission']['is_administortar'] AND !$this->user_info['permission']['is_moderator'] AND !$this->user_info['permission']['edit_question'])
			{
				...
				{
					H::redirect_msg(BOP_APP::lang()->_t('你没有权限编辑这个问题'), '/question/' . $_GET['id']);
				}
			}
		...
		{
			H::redirect_msg(BOP_APP::lang()->_t('你所在用户组没有权限发布问题'));
		}
		...
		{
			H::redirect_msg(BOP_APP::lang()->_t('你的剩余积分已经不足以进行此操作'));
		}
	...
	public function article_action()
	{
		if ($_GET['id'])
		{
			...
			{
				H::redirect_msg(BOP_APP::lang()->_t('指定文章不存在'));
			}
			...
			if (!$this->user_info['permission']['is_administortar'] AND !$this->user_info['permission']['is_moderator'] AND !$this->user_info['permission']['edit_article'])
			{
				...
				{
					H::redirect_msg(BOP_APP::lang()->_t('你没有权限编辑这个文章'), '/article/' . $_GET['id']);
				}
			}
		...
		{
			H::redirect_msg(BOP_APP::lang()->_t('你所在用户组没有权限发布文章'));
		}
	...
	public function wait_approval_action()
	{
		...
		H::redirect_msg(BOP_APP::lang()->_t('发布成功, 请等待管理员审核...'), $url);
	}
}