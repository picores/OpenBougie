...
{	
	...
	function bind_account($uinfo, $redirect, $uid, $is_ajax = false)
	{
		if (! $openid = load_class('Services_Tencent_QQConnect_V2')->get_openid())
		{
			...
			if ($is_ajax)
			{
				H::ajax_json_output(BOP_APP::RSM(null, -1, BOP_APP::lang()->_t('与 QQ 通信出错, 请重新登录')));
			}
			else
			{
				H::redirect_msg(BOP_APP::lang()->_t('与 QQ 通信出错, 请重新登录'), '/account/logout/');
			}
		}
		...
		if ($openid_info = $this->get_user_info_by_uid($uid))
		{
			if ($openid_info['name'] != $openid)
			{
				if ($is_ajax)
				{
					H::ajax_json_output(BOP_APP::RSM(null, -1, BOP_APP::lang()->_t('QQ 账号已经被其他账号绑定')));
				}
				else
				{
					H::redirect_msg(BOP_APP::lang()->_t('QQ 账号已经被其他账号绑定'), '/account/logout/');
				}
			}
		}
		...
		else if ($users_qq['uid'] != $uid)
		{
			if ($is_ajax)
			{
				H::ajax_json_output(BOP_APP::RSM(null, '-1', 'QQ 已经被其他账号绑定'));
			}
			else
			{
				H::redirect_msg(BOP_APP::lang()->_t('QQ 已经被其他账号绑定'), '/account/setting/openid/');
			}
		}
		
		$this->update_token($openid, BOP_APP::session()->QQConnect['access_token']);
