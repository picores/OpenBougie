	...
	function bind_account($uinfo, $redirect, $uid, $is_ajax = false)
	{
		if ($openid_info = $this->get_users_qq_by_uid($uid))
		{
			if ($openid_info['name'] != $uinfo['data']['name'])
			{
				if ($is_ajax)
				{
					H::ajax_json_output(BOP_APP::RSM(null, '-1', BOP_APP::lang()->_t('QQ 微博账号已经被其他账号绑定')));
				}
				else
				{
					H::redirect_msg(BOP_APP::lang()->_t('此账号已经与另外一个微博绑定'), '/account/logout/');
				}
			}
		}
		...
		else if ($users_qq['uid'] != $uid)
		{
			...
			{
				H::redirect_msg(BOP_APP::lang()->_t('此账号已经与另外一个微博绑定'), '/account/setting/openid/');
			}
		}
		...
		$this->update_token($uinfo['data']['name'], BOP_APP::session()->$oauth_access_token_name, BOP_APP::session()->$oauth_access_token_secret_name);
