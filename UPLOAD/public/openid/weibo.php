	...
	{		
		if ($openid_info = $this->get_users_sina_by_uid($uid))
		{
			if ($openid_info['id'] != $sina_profile['id'])
			{
				if ($is_ajax)
				{
					H::ajax_json_output(BOP_APP::RSM(null, '-1', BOP_APP::lang()->_t('此账号已经与另外一个微博绑定')));
				}
				else
				{
		...
		if (BOP_APP::session()->sina_token['access_token'])
		{
			$this->update_token($sina_profile['id'], BOP_APP::session()->sina_token['access_token']);
		}
