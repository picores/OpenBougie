...
class email_class extends AWS_MODEL
{
	...
	*/
	...
	public function action_email($action, $email, $link, $data = array())
	{
		...
		$email_message = (array)BOP_APP::config()->get('email_message');
	...
	public function send($email, $subject, $message, $link = null, $link_title = null)
	{
		...
		return BOP_APP::mail()->send($email, $subject, $this->get_mail_template($user_name, $subject, $message, $link, $link_title), get_setting('site_name'), $user_name);
	}
	...
	public function send_mail_queue($limit = 10)
	{
		if ($mail_queue = $this->fetch_all('mail_queue', 'is_error = 0', 'id ASC', $limit))
		{
			...
			{
				if ($error_message = BOP_APP::mail()->send($val['send_to'], $val['subject'], $val['message'], get_setting('site_name')))
				{
